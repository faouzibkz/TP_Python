...
docker run -d --name node_app_test -p 8000:8000 -v "$PWD":/app faouzibkz/nodejs_app:${version}

echo "Testing image: faouzibkz/nodejs_app:${version}"
...
curl -s http://localhost:8000 | grep "Hello World" > /dev/null
status=$?
